...
# Статус наверху
st.header("📊 Статус анализа")
col1, col2, col3, col4 = st.columns(4)

with col1:
    st.metric("Файлы загружены", f"{len(uploaded_files) if uploaded_files else 0}")
    
with col2:
    st.metric("Бриф", "✅" if uploaded_brief else "❌")
    
with col3:
    st.metric("API ключ", "✅" if api_key else "❌")
    
with col4:
    st.metric("Готовность", "✅" if (uploaded_files and api_key) else "❌")
...
with step_container:
    ...
    with st.expander("2️⃣ Загрузка данных", expanded=bool(uploaded_files or uploaded_brief)):
...
# Статус в сайдбаре (упрощенный)
st.sidebar.header("🔧 Быстрый статус")
st.sidebar.code(f"""
Файлы: {len(uploaded_files) if uploaded_files else 0}
